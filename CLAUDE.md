...
- Python 3.12 + FastAPI 0.111+, LangGraph 0.2+, LangChain, langchain-openai, langgraph-checkpoint-redis, httpx (async), asyncpg, sentence-transformers (`paraphrase-multilingual-MiniLM-L12-v2`), faiss-cpu, faster-whisper, redis, openai, pydantic v2 (002-orchestrator-domain-agents)
- Redis (LangGraph checkpointer, session state, A2A task store — 30-min session TTL, 1-hr task TTL); PostgreSQL with asyncpg (analytics DB + memory tables — read/write) (002-orchestrator-domain-agents)
...
## Recent Changes
- 002-orchestrator-domain-agents: Added Python 3.12 + FastAPI 0.111+, LangGraph 0.2+, LangChain, langchain-openai, langgraph-checkpoint-redis, httpx (async), asyncpg, sentence-transformers (`paraphrase-multilingual-MiniLM-L12-v2`), faiss-cpu, faster-whisper, redis, openai, pydantic v2
