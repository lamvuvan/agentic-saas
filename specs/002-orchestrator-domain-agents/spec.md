...
- **FR-049**: The Orchestrator MUST be implemented as a LangGraph `StateGraph(OrchestratorState)` with exactly these nodes: `classify_intent` (GPT-4o-mini, writes `intent`), `plan` (GPT-4o, writes `plan` + `plan_id`), `dispatch` (no LLM, runs `DispatchEngine`, writes `dispatch_results`), `chitchat` (GPT-4o-mini, writes `final_response`), `respond` (GPT-4o-mini, writes `final_response`). Conditional routing from `classify_intent`: `intent == "chitchat"` → `chitchat`; all others → `plan`. Fixed edges: `plan` → `dispatch` → `respond`, `chitchat` → `respond`, `respond` → `END`.
- **FR-050**: The Orchestrator graph MUST be compiled with `AsyncRedisSaver` (from `langgraph-checkpoint-redis`) as its checkpointer, `thread_id = session_id`, and a 24-hour TTL on checkpoint keys. Every request invoked via `await compiled.ainvoke(state, config={"configurable": {"thread_id": session_id}})` — conversation history (`messages`) persists across requests in the same session.
- **FR-051**: Order Agent and Customer Agent MUST each be implemented as a separate `StateGraph` compiled with `AsyncRedisSaver` (TTL 1 hour, `thread_id = a2a_task_id`). BI Agent MAY use `MemorySaver` (in-process) since it is stateless and has no HITL interrupt.
- **FR-052**: HITL for Domain Agents MUST use LangGraph's `interrupt_before=["hitl_confirm"]` on the compiled graph (NOT a manual pause flag). When the graph halts at the interrupt point: (a) `state["confirm_message"]` is read and placed in `A2ATask.input_request`; (b) A2A task status transitions to `input_required`. Resumption MUST use `await compiled.ainvoke(Command(resume={"user_confirmation": user_response}), config={"configurable": {"thread_id": task_id}})` — the graph resumes from the exact checkpoint with full prior state restored from Redis.
- **FR-053**: Each Domain Agent's LangGraph state MUST be a `TypedDict` containing: all `A2ATaskPayload` fields (`task_id`, `plan_id`, `original_message`, `instructions`, `conversation_history`, `dependency_results`), `memory_context: str` (injected by `MemoryService.retrieve()` before graph invocation), agent-specific working state fields, `confirm_message: str` (set in the node before the HITL interrupt), `user_confirmation: str` (empty until the graph resumes), and `result: dict` (populated by the `done` node).
...
- **OrchestratorState**: LangGraph `TypedDict` state for the Orchestrator graph — `session_id: str`, `tenant_id: str`, `messages: Annotated[list[BaseMessage], add_messages]` (full conversation history, persisted via checkpointer), `intent: str` (written by `classify_intent`), `plan_id: str` (written by `plan` after DB insert), `plan: dict` (`{display: {...}, routing: {steps: [...]}}` written by `plan`), `dispatch_results: dict` (sub_goal_sequence → result_dict, written by `dispatch`), `hitl_pending: dict | None` (set when a Domain Agent signals scope_change back to Orchestrator), `final_response: str` (written by `respond` or `chitchat`).
- **OrderAgentState**: LangGraph `TypedDict` state for the Order Agent graph — payload fields (`task_id`, `plan_id`, `original_message`, `instructions`, `conversation_history`, `dependency_results`), `memory_context: str`, working fields (`entities: dict`, `matched_products: list`, `customer: dict`, `order_preview: dict`), `confirm_message: str` (set in `preview_order` node before HITL interrupt), `user_confirmation: str` (empty until `Command(resume=...)` is called), `result: dict`.
- **BIAgentState**: LangGraph `TypedDict` state for the BI Agent graph — payload fields (`task_id`, `plan_id`, `original_message`, `instructions`, `conversation_history`, `dependency_results`), `memory_context: str` (sql_pattern / glossary_fix from MemoryService), `generated_sql: str` (NL2SQL output), `query_result: list` (rows from asyncpg), `result: dict`. No HITL fields — BI Agent is read-only and stateless across requests.
- **CustomerAgentState**: LangGraph `TypedDict` state for the Customer Agent graph — payload fields (`task_id`, `plan_id`, `original_message`, `instructions`, `conversation_history`, `dependency_results`), `memory_context: str` (contact_alias / customer_profile), `action: str` (`"lookup"` | `"create"` | `"update"`, set by `classify_action` node), `customer_data: dict` (tool call parameters), `confirm_message: str`, `user_confirmation: str`, `result: dict`. HITL interrupt used for `create` and `update` actions only.
